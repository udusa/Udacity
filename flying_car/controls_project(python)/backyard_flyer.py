...
class ControlsFlyer(UnityDrone):
    ...
    def local_position_callback(self):
        ...
        self.positions.append([self.local_position[0],self.local_position[1],self.local_position[2]])
        ...
        if self.flight_state == States.TAKEOFF:
            if -1.0 * self.local_position[2] > 0.95 * self.target_position[2]:
                ...
                (self.position_trajectory,
                 ...
                 self.yaw_trajectory) = self.load_test_trajectory(time_mult=0.7)

                self.trajectory = self.position_trajectory
...
if __name__ == "__main__":
    ...
    pos = np.array(drone.positions)
    traj = np.array(drone.trajectory)

    fig = plt.figure()
    ax = fig.gca(projection='3d')
    ax.plot(pos[:,0], pos[:,1], -pos[:,2],linestyle='-',marker='.',color='red')
    ax.plot(traj[:,0],traj[:,1],-traj[:,2],linestyle='-',color='blue')
